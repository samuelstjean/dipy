...
cdef _average_block(double[:, :, :] ima, int x, int y, int z, double[:, :, :] average, double weight, int rician):
...
def firdn(double[:, :] image, double[:] h):
    ...
    cdef double[:, :] filtered = np.zeros(shape=((image.shape[0] + h.shape[0]) // 2, image.shape[1]))
...
def upfir(double[:, :] image, double[:] h):
    ...
    cdef double[:, :] filtered = np.zeros(shape=(2 * image.shape[0] + h.shape[0] - 2, image.shape[1]))
                            ...
                            for ii in range(-v, v + 1):
                                ...
                                if(ni >= 0 and nj >= 0 and nk >= 0 and ni < cols and nj < rows and nk < slices):
                                    if(ima[ni, nj, nk] > 0 and (means[ni, nj, nk] > epsilon) and (variances[ni, nj, nk] > epsilon)):
                                        t1 = (means[i, j, k]) / (
                                            means[ni, nj, nk])
                                        t1i = (globalMax - means[i, j, k]) / (
                                            globalMax - means[ni, nj, nk])
                                        t2 = (variances[i, j, k]) / (
                                            variances[ni, nj, nk])
                                        if((t1 > mu1 and t1 < (1 / mu1)) or (t1i > mu1 and t1i < (1 / mu1)) and t2 > var1 and t2 < (1 / var1)):
                                            d = _distance(
                                                ima, i, j, k, ni, nj, nk, f)
