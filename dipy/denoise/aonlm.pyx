...
'''
    ...
    if(ax < 3.75):
        ...
        ans = ans * \
            (0.39894228 + y * (0.1328592e-1 + y * (0.225319e-2 + y * (-0.157565e-2 + a))))
    return ans
...
cdef _value_block(
    double[:, :, :] estimate, double[:, :, :] Label, int x, int y,
        ...
        for j in range(-f, f + 1):
            for k in range(-f, f + 1):
                ...
                if(nk2 >= sz):
                    nk2 = 2 * sz - nk2 - 1
                distancetotal += (
                    image[ni1, nj1, nk1] - image[ni2, nj2, nk2]) ** 2
                acu = acu + 1
    d = distancetotal / acu
    return d

cdef double _distance2(
    double[:, :, :] image, double[:, :, :] medias, int x, int y, int z,
        int nx, int ny, int nz, int f):
    cdef double d, acu, distancetotal
    cdef int i, j, k, ni1, nj1, ni2, nj2, nk1, nk2
    cdef int sx = image.shape[0], sy = image.shape[1], sz = image.shape[2]
