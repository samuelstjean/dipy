...
def ascm(ima, fimau, fimao, h):
    r""" Adaptive Soft Coefficients Mixing
    ...
    ----------
    ima : array
        the original (not filtered) 3D image
    fimau : array
    ...
    Notes
    -----
    In P. Coupe et al. the rician noise was simulated as
    :math:`sqrt((f+x)^2 + (y)^2)` where f is the pixel value and x and y are
    ...
    References
    -----------
    ..  [1] P Coupe, JV Manjon, M Robles, DL Collins, "Adaptive multiresolution
        non-local means filter for three-dimensional magnetic resonance image
        denoising, IET Image Processing, 2012
